...
export class EditCityComponent {
  ...
  fetchCity() {
    this.http.get<any>(`http://localhost:8080/api/cities/${this.cityId}`).subscribe((data) => {
      this.name = data.name;
  ...
  updateCity() {
    this.http
      .put(`http://localhost:8080/api/cities/${this.cityId}`, { name: this.name, population: this.population })
      .subscribe(() => {
